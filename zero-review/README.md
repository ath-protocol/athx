...
| `contracts/ath-protocol.md` | Reference → all skills | ATH Protocol v0.1 specification: endpoints, types, flows, error codes, security rules |
...
| ATH protocol flow testing | **Stable** | Any |
...
**ATH Protocol v0.1 alignment:** All skills are aware of the Agent Trust Handshake protocol. auto-dev includes a dedicated `ath-protocol-verify` phase for verifying attestation, PKCE, scope intersection, and token security compliance. auto-test includes an `ath-protocol` persona for testing the full trust handshake flow. auto-triage classifies ATH protocol issues with security-appropriate priority escalation. The protocol contract at `contracts/ath-protocol.md` is the shared reference for all skills.
