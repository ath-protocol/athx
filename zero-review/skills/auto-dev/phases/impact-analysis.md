...
- **ATH protocol compliance surface**: If the change touches ATH endpoints, attestation, scope intersection, token handling, or proxy — map which protocol invariants are in the blast radius (see `contracts/ath-protocol.md`)
...
- If ATH protocol surface is affected: all impacted protocol invariants listed, `ath-protocol-verify` phase added to sequence
