...
- **ATH protocol security**: always P0 — includes provider token leakage, attestation bypass, scope intersection returning overly broad permissions, missing PKCE validation, and token binding failures
- **ATH protocol compliance**: P1 if the service returns non-spec error codes or skips required validation steps (jti replay, audience check, agent identity mismatch); P2 if the non-compliance has no security impact
...
- ATH protocol security issues — always escalate alongside filing as P0; do not wait for triage cycle
