...
| `package.json` with `@ath-protocol/server` or `@ath-protocol/client` | `api` (ATH protocol service — use `ath-protocol` persona) |
...
| Endpoint paths matching `/ath/agents/register`, `/ath/authorize`, `/ath/token` | `api` (ATH protocol service — use `ath-protocol` persona) |
| `/.well-known/ath.json` or `/.well-known/ath-app.json` served | `api` (ATH protocol service — use `ath-protocol` persona) |
